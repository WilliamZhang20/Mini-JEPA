"""Train a self-supervised Relocate contact/reachability probe.

Relocate observations expose geometry labels directly:

- dims 30:33: palm-to-ball vector
- dims 36:39: ball-to-target vector

The probe predicts the two distances from the frozen JEPA latent. It is not a
policy and uses no action labels; evaluators can use it as a fine-state energy
when selecting flow/refined action chunks.
"""
...
def main() -> None:
    ...
    for ep in load_episodes_npz(args.episodes_npz)[: args.max_episodes]:
        ...
        if len(s) == 0 or s.shape[-1] < 39:
            continue
        palm_ball = np.linalg.norm(s[:, 30:33], axis=-1)
        ball_target = np.linalg.norm(s[:, 36:39], axis=-1)
        states.append(s)
        targets.append(np.stack([palm_ball, ball_target], axis=-1).astype(np.float32))
    ...
    print(
        json.dumps(
            {
                "event": "relocate_contact_probe_data",
                "pairs": int(len(Z)),
    ...
    for step in range(1, args.train_steps + 1):
        ...
        if step == 1 or step % 2000 == 0:
            print(json.dumps({"event": "relocate_contact_probe_train", "step": step, "loss": float(loss.detach().cpu())}), flush=True)
    ...
    print(json.dumps({"event": "relocate_contact_probe_saved", "path": str(args.out)}), flush=True)
