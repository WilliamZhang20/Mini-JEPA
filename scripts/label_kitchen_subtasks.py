...
segment is working toward. Output: an npz of (states, actions, target one-hot).
"""
...
def main() -> None:
    ...
    env = make_env("FrankaKitchen-v1", seed=0, max_episode_steps=2000)
    t2i = {t: i for i, t in enumerate(TASKS)}

    states_all, actions_all, target_all = [], [], []
    n_kept = 0
    for ep_i, ep in enumerate(ds.iterate_episodes()):
        ...
        S = np.asarray(ep.observations["observation"], np.float32)[: T + 1]
        valid = target >= 0
        if valid.sum() < 5:
            continue
        oh = np.zeros((T, 4), np.float32)
    ...
    dist = {TASKS[i]: int((allt == i).sum()) for i in range(4)} if len(allt) else {}
    print(f'{{"event": "labeled", "dataset": "{args.dataset}", "episodes_kept": {n_kept}, '
          f'"out": "{args.out}", "target_dist": {dist}}}', flush=True)
