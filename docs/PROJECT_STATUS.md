...
| Adroit Door | Learned-structure JEPA control accepted | learned subgoal net + learned actor + **predictor ranking**, 0.967/30 retained (seed 64000; predictor stays in the runtime loop per the encode/predict/plan paradigm). Predictor-free actor-only 1.00/30 — a one-episode difference; encoder/actor ablations 0.000. Matches the hand-structured schedule-phase inverse (1.00/30) with no hand-supplied structure |
| Adroit Hammer | Learned-structure JEPA control accepted | learned subgoal net + learned actor + **predictor ranking**, 0.833/30 (seed 64000). Predictor ablation 0.567; encoder/subgoal/actor ablations 0.000. Predictor-free learned control 0.733/30 — identical to the hand-structured p4 baseline as reproduced on the current stack (recorded historically as 1.00/30 on an older gymnasium-robotics) |
| Adroit Pen | SSL replacement solved | raw+latent future flow, 0.90/30; old BC removed. Learned-structure latent pipeline improved 0.367 → **0.600/30** (deterministic actor + goal-matched predictor-guided refiner + ranking; predictor ablation 0.433; reproduced across two WM scales) — still below the flow, not retained |
| Adroit Relocate | SSL replacement accepted | dual possession-specialist inverse (firm 0.045 switch) on demo-locked futures with palm-ball-emphasis reach + ball-target-emphasis held specialists, 0.957/210 held-out; former BC removed. Learned-structure latent pipeline improved 0.333 → **0.733/30** via pure scale (32k WM, 24k subgoal, 30k actor) + rank-horizons 4,8 (predictor ablation 0.467) — gap to specialists now 0.22, not retained |
