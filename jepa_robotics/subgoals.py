...
def build_fetch_pick_subgoal_artifact(
    ...
    *,
    ...
    """Extract demo-future templates without training on action labels.

    The artifact stores phase prototypes from successful-looking future states:
    gripper/object offsets and finger openings. At execution time those futures
    are re-grounded on the current object/goal and encoded by the JEPA encoder.
    """
    ...
    for ep in episodes:
        states = ep.states
        for state in states:
            phase = _classify_fetch_pick_state(state, spec)
            rows[phase].append(np.asarray(state, dtype=np.float32))
            total += 1
            if total >= max_states:
                break
        if total >= max_states:
...
def _classify_fetch_push_state(state: np.ndarray, spec: ObsSpec) -> str:
    ...
    rel = grip[:2] - obj[:2]
    behind = float(np.dot(rel, d)) < -0.020
    lateral = float(np.linalg.norm(rel - np.dot(rel, d) * d))
    low = float(abs(grip[2] - obj[2])) < 0.035
    return "push" if behind and lateral < 0.050 and low else "approach"
    ...
    *,
    ...
    for ep in episodes:
        for state in ep.states:
            phase = _classify_fetch_push_state(state, spec)
            rows[phase].append(np.asarray(state, dtype=np.float32))
            total += 1
            if total >= max_states:
                break
    ...
    return {
        "kind": "fetch_push_latent_subgoals",
        "phases": FETCH_PUSH_PHASES,
        "templates": templates,
    ...
    # Keep a small encoded demo-future bank for diagnostics and optional nearest
    # neighbour inspection. Actions are intentionally not saved.
    demo_states = np.concatenate([ep.states for ep in episodes], axis=0).astype(np.float32)
    if demo_states.shape[0] > max_states:
        idx = np.linspace(0, demo_states.shape[0] - 1, max_states).astype(np.int64)
        demo_states = demo_states[idx]
    ...
    return {
        "kind": "fetch_pick_latent_subgoals",
        "phases": FETCH_PICK_PHASES,
        "templates": templates,
        "demo_states": demo_states,
        "demo_latents": demo_latents,
...
def make_fetch_pick_target_state(raw_state: np.ndarray, spec: ObsSpec, artifact: dict) -> tuple[str, np.ndarray]:
    """Ground the next demo-derived phase future in the current scene."""
    target = np.array(raw_state, copy=True).astype(np.float32)
    obs, grip, obj, goal, finger = _fetch_parts(raw_state, spec)
    xy = float(np.linalg.norm(grip[:2] - obj[:2]))
    d3 = float(np.linalg.norm(grip - obj))
    fingers_open = float(np.sum(finger)) > 0.075
    lifted = float(obj[2]) > 0.46
    ...
    if phase in ("approach", "pregrasp"):
        ...
        # Stay open while moving around the object.
        ...
        # Move in reachable chunks toward the final desired future, not a single
        # huge teleport. The demo template supplies the grasped gripper offset.
        delta = goal - obj
