...
class JEPALatentRawExtractor(BaseFeaturesExtractor):
    """Goal-RL features = normalized raw state concatenated with frozen JEPA latent.

    A frozen encoder trained from broad exploration is useful regularization, but
    it cannot be allowed to erase the fine object/finger geometry needed by a
    critic to discover a rare contact transition.  This extractor preserves a
    direct raw-state route while supplying the predictive JEPA abstraction.
    It is the goal-env counterpart of :class:`JEPAConcatExtractor`.
    """
    ...
    def __init__(
        ...
        latent_dim = int(config["latent_dim"])
        super().__init__(observation_space, features_dim=int(spec.state_dim) + latent_dim)
        self.model = model.eval()
        for param in self.model.parameters():
            param.requires_grad_(False)
        self.register_buffer("mean", torch.as_tensor(normalizer.mean, dtype=torch.float32))
    ...
    def forward(self, observations: dict[str, torch.Tensor]) -> torch.Tensor:
        state = torch.cat(
            [observations[key].float().reshape(observations[key].shape[0], -1)
             for key in ("observation", "achieved_goal", "desired_goal")],
            dim=-1,
        )
        state = (state - self.mean.to(state.device)) / self.std.to(state.device)
